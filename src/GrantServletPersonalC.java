 ...
 */
@WebServlet("/GrantServletPersonalC")
public class GrantServletPersonalC extends HttpServlet {
...
int yes=stmt.executeUpdate("update loans set confirmper='"+ans+"',comper='"+comment+"' where id="+sendid);
if(yes==1) {
	String dt=ft.format(date);
	         ...
	         int st=stmt1.executeUpdate("update mdata set confirmtime='"+dt+"',bankerid='"+uid+"',loan_status='rejected' where id="+sendid+" and loan_type='Personal loan'");
	         if(st==1) {
	         System.out.println("Confirm Date updated for personalc loan");
	         }
	         else {
	         System.out.println("Confirm Date not updated for personalc loan");
	         }
}
       else {
         out.println("Fail");
